...
class Screens:
    ...
    def generate_screenshots(self):
        num_screenshots = int(config["screenshots_number"])
        quality = 2
        output_directory = os.path.join(self.upload_folder, "screenshots")
        os.makedirs(output_directory, exist_ok=True)

        if any(f.endswith(".png") for f in os.listdir(output_directory)):
        ...
        try:
            ...
            console.print(f"[bold red]Error probing {self.video_path}: {e.stderr.decode()}[/bold red]")
            return

        adjusted_duration = max(0, math.floor(duration) - 500)
        timestamps = [adjusted_duration / num_screenshots * i for i in range(1, num_screenshots + 1)]

        console.print("[bold yellow] ➥ Screenshots Generating...")
        for i, timestamp in enumerate(timestamps, start=1):
            ...
            try:
                ...
                console.print(f"[bold red]Error generating screenshot for {self.video_path} at {timestamp}: {e.stderr.decode()}[/bold red]")

        console.print("[bold green] ✔ Screenshots generated successfully.")

    def upload_images(self):
        image_host = config["image_host"]
        image_host_api_key = config["image_host_api_key"][image_host]
        API_URLS = {
