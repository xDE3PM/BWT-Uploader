...
class BWTorrentUploader:
    ...
    def load_metadata(self):
        ...
        category_id = metadata.get("category_id") or self.select_category()
    ...
    def parse_cookie_file(self, cookiefile="data/cookies/BWT.txt"):
        """Parse a cookies.txt file and return dict for requests."""
    ...
    async def validate_credentials(self, meta):
        cookiefile = "data/cookies/BWT.txt"
        if not os.path.exists(cookiefile):
            await self.login(cookiefile)

        vcookie = await self.validate_cookies(meta, cookiefile)
        if vcookie is not True:
            console.print('[red]Failed to validate cookies.')
            recreate = input("Log in again and create new session? (y/n): ").strip().lower() == 'y'
            if recreate:
    ...
    async def validate_cookies(self, meta, cookiefile):
        """Check if cookies from Netscape-format file are valid."""
        ...
        with requests.Session() as session:
            ...
            if "Logout" in response.text or "logout" in response.text:
                ...
                with open(cookiefile, 'w', encoding='utf-8') as f:
                    ...
                    f.write("# This file was generated by DE3PM\n")
                    f.write("# http://curl.haxx.se/rfc/cookie_spec.html\n")
                    f.write("# This is a generated file!  Do not edit.\n\n")
                    for c in session.cookies:
                        f.write(f"{c.domain}\tTRUE\t/\tFALSE\t0\t{c.name}\t{c.value}\n")
            else:
                console.print("[red][-][/red] Login failed. Check username/password.")
    ...
    def upload(self):
        ...
        with open(torrent_path, 'rb') as torrent_file:
            ...
            response = session.post(f"{self.base_url}/takeupload.php", headers=headers, cookies=cookies, files=files, data=data)
        ...
        if response.status_code == 200:
            ...
            if match:
                ...
                self._save_error_log(response.text)

        else:
            console.print(f"[bold red]Upload Failed![/bold red]")
            self._save_error_log(response.text)
            
    def _save_error_log(self, error_content):
        """Save the error HTML/text response to a file for debugging."""
        error_file = os.path.join(self.upload_folder, "upload_error.txt")
        try:
            with open(error_file, "w", encoding="utf-8") as f:
                f.write(error_content)
                console.print(f"[yellow]Error log saved to:[/yellow] {error_file}")
        except Exception as e:
            console.print(f"[red]Failed to save error log:[/red] {e}")
